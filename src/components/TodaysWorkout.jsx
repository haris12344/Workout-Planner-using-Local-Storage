...
function TodaysWorkout() {
  ...
  <h2 className="font-bold text-2xl sm:text-3xl md:text-4xl text-green-600 animate-pulse text-center mt-6">
    Today's Workouts
  </h2>
  ...
  {todayWorkouts.length > 0 ? (
    todayWorkouts.map((w) => (
      ...
      >
        <h1 className="text-lg sm:text-xl">{w.name} is planned for today</h1>
      </div>
    ))
  ) : (
  ...
  <h2 className="font-bold text-2xl sm:text-3xl md:text-4xl text-yellow-600 animate-pulse text-center mt-6">
    Tomorrow's Workouts
  </h2>
